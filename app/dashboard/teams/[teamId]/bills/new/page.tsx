...
// Mirrors server-side calculateMemberShares so preview matches what's saved to DB
function calculateLiveMemberShares(items: BillItem[], members: Member[]): MemberShare[] {
  ...
  items.forEach((item) => {
    ...
    if (item.assignedTo.length === 0) {
      ...
      // Split only among assigned members
...
export default function NewBillPage({
  ...
  const [items, setItems] = useState<BillItem[]>([
    { id: '1', name: '', price: 0, hasTax: false, assignedTo: [] },
  ])
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    setSaving(true)
    try {
      const res = await fetch(`/api/teams/${teamId}/bills`, {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({
          name,
      ...
      router.push(`/dashboard/teams/${teamId}/bills/${data.bill._id}`)
    } catch (error) {
    ...
    <div className="mx-auto max-w-4xl space-y-6">
      <div className="flex items-center gap-4">
        ...
        <div>
          ...
          <p className="mt-1 text-muted-foreground">Add a new expense to {team.name}</p>
        </div>
      </div>
      ...
      <form onSubmit={handleSubmit}>
        <div className="space-y-6">
            ...
            <CardHeader>
              <div className="flex items-center justify-between">
                <div>
                  ...
                  <CardDescription>
                    Upload a receipt image to automatically extract items (H = 13% HST)
                  </CardDescription>
                </div>
                <div>
                  <input
            ...
            <CardHeader>
              <div className="flex items-center justify-between">
                <div>
                  ...
                  <CardDescription>
                    Add items and assign them to team members. Unassigned items split equally.
                  </CardDescription>
                </div>
          ...
          <div className="flex gap-3">
            <Link href={`/dashboard/teams/${teamId}`} className="flex-1">
